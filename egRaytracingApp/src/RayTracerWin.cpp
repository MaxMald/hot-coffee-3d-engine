...
bool IsInShadow(const HitInfo& hit, const Vector3& lightPosition);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
  ...
  hc::Path rootPath = hc::Path::GetCurrentPath();
  g_assetManager.setRootPath("assets");
...
)
{
  ...
  while (depth < maxBounces)
  {
    ...
    if (closestHitInfo.pSphere == nullptr && closestHitInfo.pPlane == nullptr)
    {
    ...
    const Vector3 lightDir = (lightPosition - closestHitInfo.position).normalized();
    const Vector3 viewDir = currentRay.direction.normalized() * -1.0f;
    const Vector3 halfVector = (lightDir + viewDir).normalized();

    const REAL_TYPE NdL = max(closestHitInfo.normal.dot(lightDir), 0.0f); // Lambertian reflection
    const REAL_TYPE NdH = max(closestHitInfo.normal.dot(halfVector), 0.0f); // Blinn-Phong reflection
    ...
    const REAL_TYPE remainingEnergy = max(throughput.x, max(throughput.y, throughput.z));
    if (remainingEnergy < 0.001f)
    {
      break; // Terminate the loop if the remaining energy is very low
    }

    Vector3 reflectedDir = currentRay.direction - 2.0f * closestHitInfo.normal * currentRay.direction.dot(closestHitInfo.normal);
...
{
  Vector3 originToCenter = ray.origin - sphere.center;
  ...
  Vector3 position = ray.where(distance);
  Vector3 normal = (position - sphere.center).normalized();
...
bool IsInShadow(const HitInfo& hit, const Vector3& lightPosition)
{
  ...
  Vector3 toLight = lightPosition - shadowRay.origin;
  ...
  return (shadowHit.pSphere != nullptr || shadowHit.pPlane != nullptr);
}

void RaytraceScreen(HDC hdc, int width, int height)
{
  // Camera
  const Vector3 eyePosition = Vector3(0.0f, 1.0f, -5.0f);
  const Vector3 camTarget = Vector3(0.0f, 0.0f, 0.0f);
  const Vector3 camUp = Vector3(0.0f, 1.0f, 0.0f);

  const Vector3 forward = (camTarget - eyePosition).normalized();
  const Vector3 right = camUp.cross(forward).normalized();
  ...
  // VERSION: antisampling version + parallelization

  float samplesPerAxis = 2.0f;
...
#pragma omp parallel for 
    ...
    for (int x = 0; x < width; ++x)
    {
      ...
      for (int sy = 0; sy < samplesPerAxis; ++sy)
      {
        for (int sx = 0; sx < samplesPerAxis; ++sx)
        {
          ...
          finalColor = finalColor + FindColorForRay(ray, minRayDistance, maxRayDistance, 3);
        }
      }
  ...
  // VERSION: Normal version, no antisamples and no parallelization
